...
use violet_core::{
    style::{surface_secondary, SizeExt, StylesheetOptions},
    unit::Unit,
    widget::{bold, card, col, label, pill, raised_card, row, Rectangle, Slider},
    Edges, Widget,
};
use violet_lucide::icons::{
    LUCIDE_BRUSH, LUCIDE_CIRCLE_X, LUCIDE_LIST_TREE, LUCIDE_MAXIMIZE, LUCIDE_MINUS,
    LUCIDE_SATELLITE,
};
    ...
    row((
            ...
            window(
                ...
                col((
                        ...
                        //         )),
                        raised_card(label("lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.").with_wrap(cosmic_text::Wrap::Word))
                )),
            )
    ...
    card(
        col((
            row((
                ...
                pill(row((
                    label(LUCIDE_MAXIMIZE),
                    label(LUCIDE_MINUS),
                    label(LUCIDE_CIRCLE_X),
                ))),
            ))
            .center(),
            content,
